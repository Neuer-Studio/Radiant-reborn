...
namespace Radiant
{
    ...
    Common::Memory::Shared<Radiant::Shader> Shader::Create( const std::filesystem::path& path )
    {
        Common::Memory::Shared<Radiant::Shader> shader = nullptr;
        switch ( RendererAPI::GetAPI() )
        {
            case RenderingAPIType::OpenGL:
            {
                shader = Common::Memory::Shared<OpenGLShader>::Create( path );
            }
        }
    ...
    void ShaderLibrary::Add( const Common::Memory::Shared<Shader>& shader )
    {
    ...
    {
        auto  shader = Common::Memory::Shared<Shader>( Shader::Create( filepath ) );
        auto& name   = shader->GetShaderName();
        RADIANT_VERIFY( m_Shaders.find( name ) == m_Shaders.end(), "" );
        m_Shaders[name] = shader;
    }
    ...
    {
        RADIANT_VERIFY( m_Shaders.find( name ) == m_Shaders.end(), "" );
        m_Shaders[name] = Common::Memory::Shared<Shader>( Shader::Create( filepath ) );
    }

    const Common::Memory::Shared<Shader>& ShaderLibrary::Get( const std::string& name ) const
    {
        RADIANT_VERIFY( m_Shaders.find( name ) != m_Shaders.end(), "" );
