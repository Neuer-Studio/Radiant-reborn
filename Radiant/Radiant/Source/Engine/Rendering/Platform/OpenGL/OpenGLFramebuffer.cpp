...
namespace Radiant
{
    namespace Utils
    {
        ...
        static Common::Memory::Shared<Image2D> CreateAndAttachColorAttachment( int samples, ImageFormat format,
                                                                               uint32_t width, uint32_t height,
                                                                               int index )
        {
            Common::Memory::Shared<Image2D> image;
            ...
            glFramebufferTexture2D( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index,
                                    Utils::TextureTarget( samples > 1 ), image->GetTextureID(), 0 );
            return image;
        }
        ...
        static Common::Memory::Shared<Image2D> CreateAndAttachDepthTexture( int samples, ImageFormat format,
                                                                            uint32_t width, uint32_t height )
        {
            Common::Memory::Shared<Image2D> image;
            ...
            glFramebufferTexture2D( GL_FRAMEBUFFER, Utils::DepthAttachmentType( format ),
                                    Utils::TextureTarget( samples > 1 ), image->GetTextureID(), 0 );
            return image;
        }
    ...
    {
        Common::Memory::Shared<const OpenGLFramebuffer> instance = this;
        Rendering::SubmitCommand(
             [instance, usage]() mutable
             {
                 glBindFramebuffer( GL_FRAMEBUFFER,
                                    usage == BindUsage::Bind ? instance->m_RenderingID.value_or( 0 ) : 0 );
    ...
    void OpenGLFramebuffer::Resize( uint32_t width, uint32_t height, bool forceRecreate )
    {
        ...
        Common::Memory::Shared<OpenGLFramebuffer> instance = this;
        Rendering::SubmitCommand(
             [instance]() mutable
             {
                 ...
                 if ( instance->m_ColorAttachmentFormats.size() )
                 {
                     ...
                     for ( size_t i = 0; i < instance->m_ColorAttachments.size(); i++ )
                         instance->m_ColorAttachments[i] = Utils::CreateAndAttachColorAttachment(
                              instance->m_Specification.Samples, instance->m_ColorAttachmentFormats[i],
                              instance->m_Specification.Width, instance->m_Specification.Height, i );
                 }

                 if ( instance->m_DepthAttachmentFormat != ImageFormat::None )
                 {
                     instance->m_DepthAttachment = Utils::CreateAndAttachDepthTexture(
                          instance->m_Specification.Samples, instance->m_DepthAttachmentFormat,
                          instance->m_Specification.Width, instance->m_Specification.Height );
                 }

                 RADIANT_VERIFY( glCheckFramebufferStatus( GL_FRAMEBUFFER ) == GL_FRAMEBUFFER_COMPLETE,
