...
namespace Radiant
{
    namespace Utils
    {
        ...
        static fs::path GetBinaryPathByType( RadiantShaderType type, const fs::path& shaderFile )
        {
            switch ( type )
            {
                case RadiantShaderType::Vertex:
                    return Common::Constants::Path::RESOURCE_SPIRV_BINARY /
                           ( shaderFile.stem().string() +
                             Common::Constants::Extensions::SPIRV_BINARY_EXTENSION_VERT );
                case RadiantShaderType::Fragment:
                    return Common::Constants::Path::RESOURCE_SPIRV_BINARY /
                           ( shaderFile.stem().string() +
                             Common::Constants::Extensions::SPIRV_BINARY_EXTENSION_FRAG );
                case RadiantShaderType::Compute:
                    return Common::Constants::Path::RESOURCE_SPIRV_BINARY /
                           ( shaderFile.stem().string() +
                             Common::Constants::Extensions::SPIRV_BINARY_EXTENSION_COMP );
            }
        ...
        {
            if ( extension == Common::Constants::Extensions::SPIRV_BINARY_EXTENSION_VERT )
                return RadiantShaderType::Vertex;
            else if ( extension == Common::Constants::Extensions::SPIRV_BINARY_EXTENSION_FRAG )
                return RadiantShaderType::Fragment;
            else if ( extension == Common::Constants::Extensions::SPIRV_BINARY_EXTENSION_COMP )
        ...
        static RadiantShaderDataType SPIRTypeToShaderDataType( spirv_cross::SPIRType type )
        {
            switch ( type.basetype )
            {
                case spirv_cross::SPIRType::Boolean:
                    return RadiantShaderDataType::Bool;
                case spirv_cross::SPIRType::Int:
                    if ( type.vecsize == 1 )
                        return RadiantShaderDataType::Int;
                    if ( type.vecsize == 2 )
                        return RadiantShaderDataType::Int2;
                    if ( type.vecsize == 3 )
                        return RadiantShaderDataType::Int3;
                    if ( type.vecsize == 4 )
                        return RadiantShaderDataType::Int4;

                case spirv_cross::SPIRType::UInt:
                    return RadiantShaderDataType::UInt;
                case spirv_cross::SPIRType::Float:
                    if ( type.columns == 3 )
                        return RadiantShaderDataType::Mat3;
                    if ( type.columns == 4 )
                        return RadiantShaderDataType::Mat4;

                    if ( type.vecsize == 1 )
                        return RadiantShaderDataType::Float;
                    if ( type.vecsize == 2 )
                        return RadiantShaderDataType::Float2;
                    if ( type.vecsize == 3 )
                        return RadiantShaderDataType::Float3;
                    if ( type.vecsize == 4 )
                        return RadiantShaderDataType::Float4;
                    break;

                case spirv_cross::SPIRType::Struct:
                    return RadiantShaderDataType::Struct;
            }
    ...
    OpenGLShader::OpenGLShader( const std::filesystem::path& path ) : m_FilePath( path )
    {

        RADIANT_VERIFY( Common::Utils::FileSystem::Exists( m_FilePath ) );
        m_Name = Common::Utils::FileSystem::GetFileName( m_FilePath );
        Reload();
    }
    ...
    void OpenGLShader::Reload()
    {
        ...
        std::unordered_map<RadiantShaderType, std::vector<uint32_t>>& binarySPIRV =
             UploadFromBinaryFile( m_FilePath );
        bool statusCached = !binarySPIRV.empty();

        if ( statusCached )
        {
            for ( auto& shaderData : binarySPIRV )
            {
                ParseShaderBuffers( shaderData.first, shaderData.second );
            }
            Rendering::SubmitCommand( [this, binarySPIRV]() { Upload( binarySPIRV ); } );
        }
        else
        {
            std::string content = Common::Utils::FileSystem::ReadFileContent( m_FilePath );
            Load( content );
        }
    }
    ...
    {
        if ( !Common::Utils::FileSystem::Exists( Common::Constants::Path::RESOURCE_SPIRV_BINARY ) )
        {
            if ( !fs::exists( Common::Constants::Path::RESOURCE_SPIRV_BINARY.parent_path() ) )
            {
                fs::create_directories( Common::Constants::Path::RESOURCE_SPIRV_BINARY.parent_path() );
            }
            Common::Utils::FileSystem::CreateDirectory( Common::Constants::Path::RESOURCE_SPIRV_BINARY );
        }
    }

    std::vector<uint32_t> OpenGLShader::TryToLoadCachedData( const std::filesystem::path& path,
                                                             RadiantShaderType            type )
    {
        const auto& binaryPath = Utils::GetBinaryPathByType( type, m_FilePath );
        if ( !Common::Utils::FileSystem::Exists( binaryPath ) )
        {
            return {};
        }
        ...
        {
            return Common::Utils::FileSystem::ReadByteFileContent( binaryPath );
        }

        return {};
    }

    const std::vector<RadiantShaderType>
    OpenGLShader::ListShaderTypesByFileName( const std::string& fileName ) const
    {
        ...
        {
            if (Common::Utils::FileSystem::GetFileNameWithoutExtension( file ) == fileName )
            {
                const auto& extension = Common::Utils::FileSystem::GetFileExtension( file );
    ...
    void OpenGLShader::Load( const std::string& shader—ontent )
    {
        ...
        for ( const auto& sh : binarySPIRV )
        {
            ...
            ParseShaderBuffers( sh.first, sh.second );
        }

        Rendering::SubmitCommand( [this, binarySPIRV]() { Upload( binarySPIRV ); } );
    }

    void OpenGLShader::ParseShaderBuffers( RadiantShaderType shadertype, const std::vector<uint32_t>& data )
    {
        spirv_cross::Compiler        compiler( data );
        spirv_cross::ShaderResources res = compiler.get_shader_resources();

        RA_TRACE( "OpenGLShader::ParseShaderBuffers - {0} {1}", m_FilePath.string(),
                  Utils::RadiantShaderToString( shadertype ) );
        ...
        for ( const spirv_cross::Resource& resource : res.gl_plain_uniforms )
        {
            ...
            if ( type.basetype == spirv_cross::SPIRType::Struct )
            {
                ...
                for ( uint32_t index = 0; index < type.member_types.size(); ++index )
                {
                    ...
                    buffer = { { fullname, shadertype, dataType, 0 },
                               size,
                               offset,
                               m_UniformTotalOffset }; // Set the uniform position to 0 for now
                    offset += size;
            ...
            else
            {
                ...
                buffer = { { resource.name, shadertype, dataType, 0, arraySize },
                           size,
                           0,
                           m_UniformTotalOffset }; // Set the uniform position to 0 for now
    ...
    void OpenGLShader::ApplyShaderBuffers()
    {
        glUseProgram( m_RenderingID );

        // Apply uniform buffers
        for ( auto& [bindingPoint, buffer] : s_UniformBuffers )
        {
            if ( buffer.RenderingID == 0 )
            {
                ...
                RA_TRACE(
                     "Created (ID:{0}) Uniform Buffer at binding point {1} with name '{2}', size is {3} bytes",
                     buffer.RenderingID, buffer.Binding, buffer.Name, buffer.Size );
            ...
            else
            {
                ...
                RADIANT_VERIFY( "Resized Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes",
                                buffer.Binding, buffer.Name, buffer.Size );
            }
        }
        ...
        for ( auto& [name, uniform] : m_Uniforms )
        {
            ...
            uniform.Uniform.Binding = location;
            // Additional code can be added here to upload uniform data if needed
        }
        ...
        {
            GLint location = OGLGetUniformPosition( name.c_str() );
            glUniform1i( location, resource.Uniform.Binding );
        }
    }
    ...
    const std::unordered_map<RadiantShaderType, std::vector<uint32_t>>
    OpenGLShader::CompileToSPIR_V( const std::unordered_map<RadiantShaderType, std::string>& shaderSource )
    {
        ...
        for ( const auto source : shaderSource )
        {
            shaderc::SpvCompilationResult module =
                 compiler.CompileGlslToSpv( source.second, Utils::ShaderTypeToShader_C( source.first ),
                                            m_FilePath.string().c_str(), options );
    ...
    std::unordered_map<RadiantShaderType, std::vector<uint32_t>>
    OpenGLShader::UploadFromBinaryFile( const std::filesystem::path& path )
    {
        std::unordered_map<RadiantShaderType, std::vector<uint32_t>> outputBinary;
        std::string filename = Common::Utils::FileSystem::GetFileNameWithoutExtension( path );
    ...
    void OpenGLShader::FillBinaryFile( const std::filesystem::path& path, const std::filesystem::path& binaryPath,
    ...
    void OpenGLShader::Upload( const std::unordered_map<RadiantShaderType, std::vector<uint32_t>>& shaderBinary )
    {
        ...
        for ( const auto& kv : shaderBinary )
        {
            ...
            glShaderBinary( 1, &shaderRendererID, GL_SHADER_BINARY_FORMAT_SPIR_V, binary.data(),
    ...
    const std::unordered_map<RadiantShaderType, std::string> OpenGLShader::PreProcess( const std::string& content )
    {
        ...
        while ( pos != std::string::npos )
        {
            ...
            RADIANT_VERIFY( type == "vertex" || type == "fragment" || type == "pixel" || type == "compute",
            ...
            output[shaderType] = content.substr(
                 nextLinePos, pos - ( nextLinePos == std::string::npos ? content.size() - 1 : nextLinePos ) );
        }
    ...
    {
        Common::Memory::Shared<const OpenGLShader> instance = this;
        Rendering::SubmitCommand( [instance, use]() mutable { instance->RT_Use( use ); } );
    }
