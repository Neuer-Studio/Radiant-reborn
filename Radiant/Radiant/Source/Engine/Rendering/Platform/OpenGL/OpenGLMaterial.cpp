...
namespace Radiant
{
    OpenGLMaterial::OpenGLMaterial( const Common::Memory::Shared<Shader>& shader ) : m_Shader( shader )
    {
    }
    ...
    void OpenGLMaterial::UploadImageToShader( const ImageDescriptor&          imageDescriptor,
                                              Common::Memory::Shared<Image2D> image2D ) const
    {
    ...
    {
        Common::Memory::Shared<const OpenGLMaterial> instance( this );
    ...
    void OpenGLMaterial::SetImage2D( const ImageDescriptor&                 descriptor,
                                     const Common::Memory::Shared<Image2D>& image2D ) const
    {
        if ( !image2D )
            return;
    ...
    void OpenGLMaterial::SetImage2D( const ImageDescriptor&                   descriptor,
                                     const Common::Memory::Shared<Texture2D>& texture2D ) const
    {
        if ( !texture2D )
            return;
