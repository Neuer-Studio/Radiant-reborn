...
namespace Radiant
{
    ...
    {
        SetProjectionMatrix( glm::perspectiveFov( glm::radians( m_VerticalFOV ), float( width ), float( height ),
                                                  m_NearPlane, m_FarPlane ) );
    }

    void Camera::OnUpdate( Common::Timestep ts )
    {
        using namespace Common;

        const void* window = Application::GetInstance().GetWindow()->GetNativeWindow();

        const glm::vec2& MousePosition{ Input::Mouse::Get().GetMouseX( window ),
                                        Input::Mouse::Get().GetMouseY( window ) };
        const glm::vec2  MouseDelta = ( MousePosition - m_InitialMousePosition ) * 0.002f;

        if ( Input::Mouse::Get().IsMouseButtonPressed( MouseButton::Right, window ) )
        {
            Input::Mouse::Get().SetCursorMode( Input::MouseState::Locked, window );

            const float YawSign       = GetUpDirection().y < 0 ? -1.0f : 1.0f;
            const float CameraSpeed   = 0.002f * ts.GetMilliseconds();
            const float RotationSpeed = 0.3f * ts.GetMilliseconds();

            if ( Input::Keyboard::IsKeyPressed( KeyCode::Q, window ) )
                m_LocationDelta -= CameraSpeed * glm::vec3{ 0.f, YawSign, 0.f };
            if ( Input::Keyboard::IsKeyPressed( KeyCode::E, window ) )
                m_LocationDelta += CameraSpeed * glm::vec3{ 0.f, YawSign, 0.f };
            if ( Input::Keyboard::IsKeyPressed( KeyCode::S, window ) )
                m_LocationDelta -= CameraSpeed * m_Direction;
            if ( Input::Keyboard::IsKeyPressed( KeyCode::W, window ) )
                m_LocationDelta += CameraSpeed * m_Direction;
            if ( Input::Keyboard::IsKeyPressed( KeyCode::A, window ) )
                m_LocationDelta -= CameraSpeed * m_RightDirection;
            if ( Input::Keyboard::IsKeyPressed( KeyCode::D, window ) )
                m_LocationDelta += CameraSpeed * m_RightDirection;
            ...
            m_Direction = glm::rotate(
                 glm::normalize( glm::cross( glm::angleAxis( -m_PitchDelta, m_RightDirection ),
        ...
        {
            Input::Mouse::Get().SetCursorMode( Input::MouseState::Visible, window );
        }
    ...
    void Camera::OnEvent( Common::Event& e )
    {
        ...
        eventManager.Notify<Common::EventWindowResize>(
             [this]( const Common::EventWindowResize& event )
             {
