...
namespace Radiant
{
    ...
    {
        template <typename T>
        static std::optional<T> GetData( const Common::Deserialization::NodeReader& node,
                                         const std::string& nodeName, const std::string& valueName )
        {
        ...
        static std::optional<std::filesystem::path> GetAssetPath( const Common::Deserialization::NodeReader& node )
        {
            return GetData<std::string>( node, "AssetPath", "Asset" );
        }

        static std::optional<std::string> GetTag( const Common::Deserialization::NodeReader& node )
        {
            return GetData<std::string>( node, "TagComponent", "Tag" );
        }

        static Entity CreateEntity( Common::Memory::Weak<Scene>                scene,
                                    const Common::Deserialization::NodeReader& node, const std::any& value )
        {
            uint64_t uuidValue = std::stoull( std::any_cast<std::string>( value ) );
            Common::UUID     uuid( uuidValue );
    ...
    {
        const auto sdfsdf = Common::Memory::Shared<Assets::AssetManager>::Create();
    }
    ...
    [[nodiscard]] Entity Scene::CreateEntityWithID( const Common::UUID& uuid, const std::string& name )
    {
    ...
    std::optional<Radiant::Entity> Scene::TryGetEntityWithUUID( const Common::UUID& uuid ) const
    {
    ...
    void Scene::SubmitMesh( const Common::Memory::Shared<Mesh>&          mesh,
    ...
    const Common::Memory::Shared<Radiant::Image2D>& Scene::GetFinalPassImage() const
    {
    ...
    [[nodiscard]] Radiant::Entity Scene::InstantiateMesh( const Common::Memory::Shared<Mesh>& mesh,
    ...
    void Scene::BuildMeshEntityHierarchy( const Entity&                               rootEntity,
    ...
    Scene::GetModelSpaceBoneTransforms( const std::vector<Common::UUID>&                    boneEntityIds,
                                        const Common::Memory::Shared<AnimatedMesh>& mesh )
    {
    ...
    void Scene::UpdateAnimation( Common::Timestep ts )
    {
        const auto& view =
                                                        ...
                                                        const Common::Memory::Shared<AnimatedMesh>& mesh )
    {
        ...
        if ( mesh )
        {
            ...
            for ( const auto& boneInfo : bonesInfo )
            {
                ...
                boneEntityIds.emplace_back( e ? e->GetUUID() : Common::UUID( 0 ) );
            }
        }
        return boneEntityIds;
    }

    Common::serialized_str SceneSerialize::GetSerializedScene_STRING( Common::Memory::Weak<Scene> scene )
    {
    ...
    std::optional<Common::Memory::Shared<Radiant::Scene>>
    SceneDeserialize::GetDeserializedScene_Object( const Common::serialized_str& context )
    {
        using namespace Common;
