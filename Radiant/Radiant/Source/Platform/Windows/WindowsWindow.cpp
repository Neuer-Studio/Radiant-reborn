...
namespace Radiant
{
    ...
    WindowsWindow::WindowsWindow( const Common::WindowSpecification& specification )
    {
        ...
        if ( specification.Fullscreen )
        {
            ...
            m_Window = glfwCreateWindow( videoMode->width, videoMode->height, m_Data.Specification.Title.c_str(),
                                         primaryMonitor, nullptr );
        }

        else
        {
            m_Window = glfwCreateWindow( (int)m_Data.Specification.Width, (int)m_Data.Specification.Height,
                                         m_Data.Specification.Title.c_str(), nullptr, nullptr );
            /*	int max_width = GetSystemMetrics(SM_CXSCREEN);
                    int max_hieght = GetSystemMetrics(SM_CYSCREEN);

                    glfwSetWindowMonitor(m_Window, NULL, (max_width / 2) - (m_Data.Specification.Width / 2),
               (max_hieght / 2) - (m_Data.Specification.Height / 2), m_Data.Specification.Width,
               m_Data.Specification.Height, GLFW_DONT_CARE);*/
        }

        RA_INFO( "[Window(Windows)] Creating window {} ({}, {})", m_Data.Specification.Title,
                 m_Data.Specification.Width, m_Data.Specification.Height );
        ...
        glfwSetWindowSizeCallback( m_Window,
                                   ...
                                   {
                                       auto&             data = *(WindowData*)glfwGetWindowUserPointer( window );
                                       Common::EventWindowResize event( (unsigned int)width, (unsigned int)height );
                                       data.EventCallback( event );
                                   } );
        ...
        glfwSetMouseButtonCallback( m_Window,
                                    []( GLFWwindow* window, int button, int action, int mods )
                                    {
                                        ...
                                        switch ( action )
                                        {
                                            case GLFW_PRESS:
                                            {
                                                Common::MouseButtonPressedEvent event( (Common::MouseButton)button );
                                                data.EventCallback( event );
                                                break;
                                            }
                                        }
