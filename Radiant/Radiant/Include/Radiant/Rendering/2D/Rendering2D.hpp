...
namespace Radiant
{
    ...
    private:
        struct Lines
        {
            ...
            Common::Memory::Shared<VertexBuffer> LineVertexBuffer;
            Common::Memory::Shared<IndexBuffer>  LineIndexBuffer;
            Common::Memory::Shared<Pipeline>     LinePipeline;
            std::vector<LineVertex>              LineVertexData;
            uint32_t                             LinesCount;
            float                                LineWidth;

            const Common::Memory::Shared<Shader>& GetShader() const
            {
                return LinePipeline->GetSpecification().Shader;
            }
        };
