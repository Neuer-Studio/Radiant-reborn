...
namespace Radiant
{
	...
	Memory::Shared<Texture2D> Texture2D::Create(const Texture2DCreateInformation& info)
	{
		switch (RendererAPI::GetAPI())
		{
			case RenderingAPIType::None:    return nullptr;
			case RenderingAPIType::OpenGL:  return Memory::Shared<OpenGLTexture2D>::Create(info);
		}
		RADIANT_VERIFY(false, "Unknown Rendering API");
		return nullptr;
	}
