...
namespace Radiant
{
	...
	void OpenGLShader::Load(const std::string& shaderÑontent)
	{
		PreProcess(shaderÑontent);
	...
	void OpenGLShader::ParseBuffers(RadiantShaderType shadertype, const std::vector<uint32_t>& data)
	{
		...
		for (const spirv_cross::Resource& resource : res.uniform_buffers)
		{
			...
			if (m_UniformBuffers.find(bufferName) == m_UniformBuffers.end())
			{
				...
				glBindBufferBase(GL_UNIFORM_BUFFER, buffer.Binding, buffer.RenderingID); //TODO: Èçìåíÿòü â use() RenderingID
