...
namespace Radiant
{
	namespace Utils
	{
		...
		[[nodiscard]] static bool isMultiSampled(uint32_t samples)
		{
			return samples > 1;
		}
		...
		static GLuint RadiantTexTypeToOGL(TextureRendererType type, uint32_t samples)
		{
			switch (type)
			{
			case TextureRendererType::Texture2D:
			{
				if (isMultiSampled(samples))
				{
	...
	void OpenGLImage2D::Invalidate()
	{
		...
		auto texType = Utils::RadiantTexTypeToOGL(m_Specification.Type, m_Specification.TextureSamples);
		glCreateTextures(texType, 1, &m_RenderingID);

		auto internalformat = Utils::RadiantInternalFormatToOGL(m_Specification.Format);
		if(Utils::isMultiSampled(m_Specification.TextureSamples))
			glTextureStorage2DMultisample(m_RenderingID, m_Specification.TextureSamples, internalformat, m_Specification.Width, m_Specification.Height, GL_FALSE);
		else
			glTextureStorage2D(m_RenderingID, m_MipmapLevels, internalformat, m_Specification.Width, m_Specification.Height);

		if (RA_HAS_VALUE(m_Specification.Data))
		{ 
			auto format = Utils::RadiantFormatToOGL(m_Specification.Format);
			auto type = Utils::OGLDataType(m_Specification.Format);

			glTextureSubImage2D(m_RenderingID, 0, 0, 0, m_Specification.Width, m_Specification.Height, format, type, RA_GET_VALUE(m_Specification.Data));
			glGenerateTextureMipmap(m_RenderingID); // TODO: optional
		}
		glCreateSamplers(1, &m_SamplerRendererID);
		glSamplerParameteri(m_SamplerRendererID, GL_TEXTURE_MIN_FILTER, (m_Specification.Data || m_MipmapLevels > 1) ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
