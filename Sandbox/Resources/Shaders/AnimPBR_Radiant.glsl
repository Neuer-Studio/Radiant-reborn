...
layout (location = 21) uniform mat4 u_BoneTransform[100];
...
void main()
{
    ...
    for(int i = 0 ; i < MAX_BONE_INFLUENCE ; i++)
    {
        if(a_BoneIndices[i] == -1) 
            continue;
        if(a_BoneIndices[i] >=MAX_BONES) 
        {
            totalPosition = vec4(a_Position, 1.0f);
            break;
        }
        vec4 localPosition = u_BoneTransform[a_BoneIndices[i]] * vec4(a_Position, 1.0f);
        totalPosition += localPosition * a_BoneWeights[i];
       // vec3 localNormal = mat3(finalBonesMatrices[a_BoneIndices[i]]) * a_Normals;
    }
    ...
    gl_Position =  u_ViewProjectionMatrix * u_Transform * (totalPosition);
}
